...
export async function readFile(id, filePath) {
  ...
  try {
    ...
    // Check file size first (reject > 2MB)
    const sizeOut = await execInContainer(
      workspace.containerName,
      `stat -c %s ${JSON.stringify(absPath)} 2>/dev/null`,
    );
    if (sizeOut !== null) {
    ...
    // Check if binary
    const mimeOut = await execInContainer(
      workspace.containerName,
      `file --mime-type -b ${JSON.stringify(absPath)} 2>/dev/null`,
    );
