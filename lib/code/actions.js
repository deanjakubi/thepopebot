...
import { getForwards, removeForward } from './port-forwards.js';
...
export async function closeInteractiveMode(id, isClean) {
  ...
  try {
    ...
    // Clean up port forwards and Traefik routes
    try {
      const { stopScanning } = await import('./port-scanner.js');
      stopScanning(id, true);
    } catch {}
 ...
 * @param {string} id - Workspace ID
 * @returns {Promise<{success: boolean, ports?: Array<{port: number, url: string, detectedAt: number}>}>}
 */
export async function listPortForwards(id) {
  ...
  const forwards = getForwards(id);
  const hostname = process.env.APP_HOSTNAME;
  const ports = [];

  for (const [port, data] of forwards) {
    const url = hostname
      ? `https://${id}-${port}.${hostname}`
      : null;
    ports.push({ port, url, detectedAt: data.detectedAt });
  }
 ...
 * @returns {Promise<{success: boolean}>}
 */
export async function removePortForward(id, port) {
  const user = await requireAuth();
  const workspace = getCodeWorkspaceById(id);
  if (!workspace || workspace.userId !== user.id) {
    return { success: false };
  ...
  removeForward(id, port);

  // Update Traefik config
  try {
    const { writeTraefikConfig } = await import('./port-scanner.js');
    writeTraefikConfig();
  } catch {}
