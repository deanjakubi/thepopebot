...
async function* chatStream(threadId, message, attachments = [], options = {}) {
  ...
  if (useSDK) {
    ...
    yield { type: 'tool-call', toolCallId: setupToolCallId, toolName: 'workspace_setup', args: setupArgs };

    try {
      await ensureWorkspaceRepo({ workspaceDir: repoDir, repo, branch, featureBranch });
      ensureSkills(repoDir, isCodeMode ? 'code' : 'agent');
      yield { type: 'tool-result', toolCallId: setupToolCallId, result: `Workspace ready on ${featureBranch || branch}` };
    } catch (err) {
