...
export async function getCodingAgentSettings() {
  ...
  try {
    ...
    const codexCliEnabled = getConfig('CODING_AGENT_CODEX_ENABLED');
    const codexCliAuth = getConfig('CODING_AGENT_CODEX_AUTH');
    const codexCliModel = getConfig('CODING_AGENT_CODEX_MODEL') || '';
...
export async function updateCodingAgentConfig(agent, config) {
  ...
  try {
    ...
    if (agent === 'claude-code') {
      ...
      if (config.enabled !== undefined) setConfigValue('CODING_AGENT_CODEX_ENABLED', String(config.enabled));
      if (config.auth !== undefined) setConfigValue('CODING_AGENT_CODEX_AUTH', config.auth);
      if (config.model !== undefined) setConfigValue('CODING_AGENT_CODEX_MODEL', config.model);
