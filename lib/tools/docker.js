...
async function runContainer({ containerName, image, env = [], workingDir, hostConfig = {} }) {
  ...
  if (inspectRes.status !== 200) {
    ...
    // Verify the image actually landed — pull streams NDJSON so HTTP 200 doesn't guarantee success
    const verifyRes = await dockerApi('GET', `/images/${encodeURIComponent(image)}/json`);
    if (verifyRes.status !== 200) {
      throw new Error(`Docker pull failed: image ${image} not found after pull`);
    }
  }
