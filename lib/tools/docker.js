 ...
 * @param {string} agent - 'claude-code', 'pi-coding-agent', 'gemini-cli', 'codex', 'opencode'
...
function buildAgentAuthEnv(agent) {
  ...
  if (agent === 'claude-code') {
    ...
    backendApi = 'openai';
    const authMode = getConfig('CODING_AGENT_CODEX_AUTH') || 'api-key';
    if (authMode === 'oauth') {
    ...
    const model = getConfig('CODING_AGENT_CODEX_MODEL');
    if (model) env.push(`LLM_MODEL=${model}`);
  }
